...
from database import init_db, get_user, create_user, save_history, get_history, clear_history, activate_subscription, check_flood, check_daily_limit
    ...
    try:
        ...
        if user_id != ADMIN_ID:
            ...
            if not await check_daily_limit(user_id, limit=200):
                await update.message.reply_text("❌ Дневной лимит (200) исчерпан.")
                return
    ...
    try:
        ...
        if user_id != ADMIN_ID:
            if not await check_flood(user_id, 3):
                await update.message.reply_text("⏳ Подожди 3 секунды.")
                return
            if not await check_daily_limit(user_id, 200):
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "solve":
        ...
        await query.edit_message_text("❓ Текст, фото, голос, график x**2")
